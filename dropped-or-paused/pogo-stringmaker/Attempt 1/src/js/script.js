$(document).ready(function () {
	GoToAnchor = function () {
		...
		selected_anchor_name = encodeURIComponent(
		...
		if (!$(this).hasClass("tabs-processed")) {
			...
			if ($(this).find(".tab-panel").length === 0) {
				my_tablabels.each(function (i) {
					...
					kids = $(this)
						.nextUntil(".tab-label")
						.wrapAll('<div class="tab-panel"></div>');
...
for (i = 0; i < coll.length; i++) {
	coll[i].addEventListener("click", function () {
		...
		if (content.style.maxHeight) {
			...
			adjustHeight(content, false);
		} else {
			content.style.maxHeight = content.scrollHeight + "px";
			adjustHeight(content, true);
		}
	});
}

function adjustHeight(content, isExpanding) {
